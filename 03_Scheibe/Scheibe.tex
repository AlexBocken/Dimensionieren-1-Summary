\section{Scheibe}
    ...
    \subsection{Annahmen}
        ...
        $\rightarrow$Definiere $F(x,y)$ (ayrische Spannungsfkt) mit $\sigma_{xx}=F_{,yy}$; $\sigma_{yy}=F_{,xx}$; $\tau_{xy}=-F_{,xy}$. Aus Kompatibilitätsbedingung: $\varepsilon_{xx,yy}+\varepsilon_{yy,xx}=2\varepsilon_{xy,xy} \rightarrow \Delta\Delta F=0$. Funktion F(x,y) so wählen, damit RB erfüllt.
    ...
    \subsection{Scheibe mit Riss}
        ...
        \begin{center}
            \vspace{-2mm}
            \includegraphics[width=70mm, height= 25mm]{03/Scheiberiss}
        \end{center}